...
Page {
    ...
    function ruleName(id) {
        // Translated by id (retranslate-safe lookup property below); the
        // C++ name is only a fallback for unknown rules.
        return page.ruleI18n[id] || id
    }

    // Translation lookup keyed by rule id. qsTr lives in this property
    // binding so a live language switch re-evaluates it and every binding
    // that depends on ruleName()/ruleDescription().
    readonly property var ruleI18n: ({
        "user-trash": qsTr("Trash"),
        "apt-cache": qsTr("APT package cache"),
        "thumbnail-cache": qsTr("Thumbnail cache")
    })
    readonly property var ruleI18nDesc: ({
        "user-trash": qsTr("Files moved to the XDG home trash. Restorable until the trash is emptied."),
        "apt-cache": qsTr("Deb packages downloaded to /var/cache/apt/archives by apt installs and upgrades."),
        "thumbnail-cache": qsTr("Cached thumbnails generated by file managers for image and video previews.")
    })

    function ruleDescription(id) {
        return page.ruleI18nDesc[id] || ""
    }
                        ...
                        RowLayout {
                            ...
                            ColumnLayout {
                                ...
                                RowLayout {
                                    ...
                                    Text {
                                        text: page.ruleName(modelData.id)
                                ...
                                Text {
                                    text: page.ruleDescription(modelData.id) !== ""
                                          ? page.ruleDescription(modelData.id)
        ...
        function openReview() {
            ...
            for (let i = 0; i < rules.length; ++i) {
                if (page.isSelected(rules[i].id)) {
                    ...
                    rows.push({
                        label: page.ruleName(rules[i].id),
